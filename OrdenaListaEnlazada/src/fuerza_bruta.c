...
void ordenar_fuerza_bruta(Persona** ref_cabeza) {
    if (*ref_cabeza == NULL) return; // Si la lista esta vacia, no hay nada que ordenar
    ...
    do {
        ...
        ptr1 = *ref_cabeza; // Empezamos desde la cabeza de la lista

        while (ptr1->siguiente != lptr) { // Recorremos hasta el elemento ya ordenado
            if (ptr1->edad > ptr1->siguiente->edad) { // Comparar edades adyacentes
                // Intercambiar datos entre nodos sin romper los punteros
                char nombre_temporal[32];
                int temp_edad = ptr1->edad;
                strcpy(nombre_temporal, ptr1->nombre); // Respaldar nodo 1
                
                ptr1->edad = ptr1->siguiente->edad;   // Mover nodo 2 al nodo 1
                strcpy(ptr1->nombre, ptr1->siguiente->nombre);
                
                ptr1->siguiente->edad = temp_edad;    // Asignar respaldo al nodo 2
                strcpy(ptr1->siguiente->nombre, nombre_temporal);
                
                swapped = true; // Hubo al menos un intercambio
            }
            ptr1 = ptr1->siguiente; // Avanzar al siguiente nodo
        }
